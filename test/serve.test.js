...
describe('Serve SPA', () => {
  ...
  it('gets the full settings and deploys html', async () => {
    const createdServer = await startServer(app, allowedExt, 0, true, 'test/mocks/dist/test-setting-hack', 'test/mocks/settings.json');
  ...
  it('routes not allowed', async () => {
    const createdServer = await startServer(app, allowedExt, 0, false, 'test/mocks/dist/test-setting-hack', 'test/mocks/settings.json');
    server = createdServer.server;
  ...
  it('not settings file', async () => {
    const createdServer = await startServer(app, allowedExt, 0, false, 'test/mocks/dist/test-setting-hack');
    server = createdServer.server;
  ...
  it('error in promise', async () => {
    ...
    await startServer(app, allowedExt, 0, false, 'test/mocks/dist/test-setting-hack', 'test/mocks/things.json');
